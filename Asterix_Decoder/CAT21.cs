...
namespace Class_Library
{
    public class CAT21
    {
                        ...
                        if (FSPEC.Length > 24)
                        {
                            ...
                            if (FSPEC.Length > 32)
                            {
                                ...
                                if (FSPEC.Length > 40)
                                {
                                    ...
                                    if (FSPEC.Length > 48)
                                    {
                                        if (FSPEC[48] == '1') { Position = this.Decode_Receiver_ID(Position, CAT21_Message); }
                                    }
                                }
                            }
                        }
        ...
        {
            if (CAT21_Message[Position].Last() == '0') { RP = "Data Driven Mode"; }

            else
            {
                double RP_double = Convert.ToInt32(CAT21_Message[Position], 2) * 0.5 - 0.5;
        ...
        {
            float ToMR_Position_seconds = Convert.ToInt32(CAT21_Message[Position] + CAT21_Message[Position + 1] + CAT21_Message[Position + 2], 2) * 1 / 128;
        ...
        {
            float ToMR_Velocity_seconds = Convert.ToInt32(CAT21_Message[Position] + CAT21_Message[Position + 1] + CAT21_Message[Position + 2], 2) * 1 / 128;
        ...
        {
            float ToART_seconds = Convert.ToInt32(CAT21_Message[Position] + CAT21_Message[Position + 1] + CAT21_Message[Position + 2], 2) * 1 / 128;

            double ToART_hours = ToART_seconds / 3660;

            int ToART_minutes = Convert.ToInt32(ToART_hours % 60);

            ToART_hours = Convert.ToInt32(ToART_hours);
        ...
        private int Decode_Position_WGS(int Position, string[] CAT21_Message)
        {
            ...
            if (Lat_WGS84_bin[0] == '0') { Lat_WGS84 = Convert.ToString(Convert.ToInt32(Lat_WGS84_bin, 2)); }
            if (Lat_WGS84_bin[0] == '1')
            {
                Lat_WGS84 = Convert.ToString(Library.twos_complement(Lat_WGS84_bin) * (180 / Math.Pow(2, 23)));
            }
            if (Lon_WGS84_bin[0] == '0') { Lon_WGS84 = Convert.ToString(Convert.ToInt32(Lon_WGS84_bin, 2)); }
        ...
        private int Decode_Position_WGS84(int Position, string[] CAT10_Message)
        {
            ...
            if (Lat_WGS84_HP_bin[0] == '0') { Lat_WGS84_HP = Convert.ToString(Convert.ToInt32(Lat_WGS84_HP_bin, 2)); }
            if (Lat_WGS84_HP_bin[0] == '1')
            {
                Lat_WGS84_HP = Convert.ToString(Library.twos_complement(Lat_WGS84_HP_bin) * (180 / Math.Pow(2, 30)));
            }
            if (Lon_WGS84_HP_bin[0] == '0') { Lon_WGS84_HP = Convert.ToString(Convert.ToInt32(Lon_WGS84_HP_bin, 2)); }
        ...
        {
            MAM = Convert.ToString(Convert.ToInt32(CAT21_Message[Position], 2)) + " dBm";
        ...
        private int Decode_Flight_Level(int Position, string[] CAT21_Message)
        {
            ...
            if (FL_bin[0] == '0') { FL = Convert.ToString(Convert.ToInt32(FL_bin, 2) * 0.25); }
        ...
        private int Decode_Data_Ages(int Position, string[] CAT21_Message)
        {
            ...
            {
                if (Field_Presence[i] == "1") { Fields[i] = Convert.ToString(Convert.ToInt32(CAT21_Message[Position + Count], 2) * 0.1); Count++; }
