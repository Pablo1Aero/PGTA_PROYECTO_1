...
namespace Class_Library
{
    public class CAT10
    {
        ...
        public CAT10(string[] CAT10_Message, DecodeLibrary Library)
        {
            ...
            if (FSPEC[7] == '1') { Position = this.Decode_Flight_Level(Position, CAT10_Message); }

                if (FSPEC.Length > 8)
                {
                    if (FSPEC[8] == '1') { Position = this.Decode_Measured_Height(Position, CAT10_Message); }
                    if (FSPEC[9] == '1') { Position = this.Decode_PAM(Position, CAT10_Message); }
                    if (FSPEC[10] == '1') { Position = this.Decode_Time_of_Day(Position, CAT10_Message); }
                    if (FSPEC[11] == '1') { Position = this.Decode_Track_Number(Position, CAT10_Message); }
                    if (FSPEC[12] == '1') { Position = this.Decode_Track_Status(Position, CAT10_Message); }
                    if (FSPEC[13] == '1') { Position = this.Decode_Track_Velocity_Polar_Coordinates(Position, CAT10_Message); }
                    if (FSPEC[14] == '1') { Position = this.Decode_Track_Velocity_Cartesian_Coordinates(Position, CAT10_Message); }
                    if (FSPEC[15] == '1') { Position = this.Decode_Calculated_Acceleration(Position, CAT10_Message); }

                    if (FSPEC.Length > 16)
                    {
                        if (FSPEC[16] == '1') { Position = this.Decode_Target_Address(Position, CAT10_Message); }
                        if (FSPEC[17] == '1') { Position = this.Decode_Target_Identification(Position, CAT10_Message); }
                        if (FSPEC[18] == '1') { Position = this.Decode_Mode_S_MB_Data(Position, CAT10_Message); }
                        if (FSPEC[19] == '1') { Position = this.Decode_Size_Orientation(Position, CAT10_Message); }
                        if (FSPEC[20] == '1') { Position = this.Decode_Presence(Position, CAT10_Message); }
                        if (FSPEC[21] == '1') { Position = this.Decode_Vehicle_Fleet_Identification(Position, CAT10_Message); }
                        if (FSPEC[22] == '1') { Position = this.Decode_Pre_Programmed_Message(Position, CAT10_Message); }
                        if (FSPEC[23] == '1') { Position = this.Decode_Standard_deviation_of_position(Position, CAT10_Message); }

                        if (FSPEC.Length > 24)
                        {
                            if (FSPEC[24] == '1') { Position = this.Decode_System_Status(Position, CAT10_Message); }
                        }
                    }
                }
        }        
        ...
        private int Decode_Measured_Position_Polar(int Position, string[] CAT10_Message)
        {
            ...
            Position = Position++;   //Deberia estar esto?
            double Measured_Theta = Convert.ToDouble(Convert.ToInt32(Convert.ToString(CAT10_Message[Position] + CAT10_Message[Position + 1]), 2)) * 0.0055;//(360/(Math.Pow(2, 16)));
        ...
        private int Decode_Cartesian_Position(int Position, string[] CAT10_Message)
        {
            ...
            if (Y_Component_bin[0] == '0') { Y_Component = Convert.ToString(Convert.ToInt32(X_Component_bin, 2)); }
            if (Y_Component_bin[0] == '1') { Y_Component = Convert.ToString(Library.twos_complement(X_Component_bin)); }
        ...
        #region Item I010/090 Flight Level in Binary Representation DA FALLOooooooooooooooooooooooooooooooooooooooooooooooooo
        ...
        private int Decode_Flight_Level(int Position, string[] CAT10_Message)
        {
            ...
            int FL_int = Library.twos_complement(Convert.ToString(String.Concat(CAT10_Message[Position][2], CAT10_Message[Position][3], CAT10_Message[Position][4], CAT10_Message[Position][5], CAT10_Message[Position][6], CAT10_Message[Position][7]) + CAT10_Message[Position + 1])); ;

            FL = Convert.ToString(Convert.ToInt32(FL_int) * 0.25);
        ...
        private int Decode_Track_Number(int Position, string[] CAT10_Message)
        {
            ...
            string Track_Number = Convert.ToString(Convert.ToInt32(bin_Track_Number, 2));
        ...
        private int Decode_Track_Status(int Position, string[] CAT10_Message)
        {
            ...
            if (TRE_bin == "0") { TRE = "Defaut"; }
            if (TRE_bin == "1") { TRE = "Last report track"; }

            string CST_bin = Convert.ToString(CAT10_Message[Position][2] + CAT10_Message[Position][3]);
            ...
            if (STH_bin == "0") { MAH = "Measured position"; }
            if (STH_bin == "1") { MAH = "Smoothed position"; }

            Position++;
            while (CAT10_Message[Position - 1].Last() == 1)
            {
                string TOM_bin = Convert.ToString(CAT10_Message[Position][0] + CAT10_Message[Position][1]);
                ...
                string DOU_bin = Convert.ToString(CAT10_Message[Position][2] + CAT10_Message[Position][3] + CAT10_Message[Position][4]);
                ...
                string MRS_bin = Convert.ToString(CAT10_Message[Position][2]);
        ...
        private int Decode_Track_Velocity_Polar_Coordinates(int Position, string[] CAT10_Message)
        {
            ...
            Track_Angle = Convert.ToString(Convert.ToInt32(Track_Angle_bin, 2)*0.005); //º
            Position = Position + 2;
        ...
        private int Decode_Track_Velocity_Cartesian_Coordinates(int Position, string[] CAT10_Message)
        {
            ...
            if (Ground_Speed_bin[0] == '0') { Vy = Convert.ToString(Convert.ToInt32(Track_Angle_bin, 2) * (0.25)); }
            if (Ground_Speed_bin[0] == '1') { Vy = Convert.ToString(Library.twos_complement(Track_Angle_bin) * (0.25)); }
        ...
        {
            string STI_bin = Convert.ToString(CAT10_Message[Position][0] + CAT10_Message[Position][1]);
            ...
            string Six1 = Convert.ToString(CAT10_Message[Position][0] + CAT10_Message[Position][1] + CAT10_Message[Position][2] + CAT10_Message[Position][3] + CAT10_Message[Position][4] + CAT10_Message[Position][5]);
            Char1 = Library.Target_Identification_Coding(Six1);
            Position = Position + 1;
            string Six2 = Convert.ToString(CAT10_Message[Position-1][6] + CAT10_Message[Position-1][7] + CAT10_Message[Position][0] + CAT10_Message[Position][1] + CAT10_Message[Position][2] + CAT10_Message[Position][3]);
            Char2 = Library.Target_Identification_Coding(Six2);
            Position = Position + 1;
            string Six3 = Convert.ToString(CAT10_Message[Position-1][4] + CAT10_Message[Position-1][5] + CAT10_Message[Position-1][6] + CAT10_Message[Position-1][7] + CAT10_Message[Position][0] + CAT10_Message[Position][1]);
            Char3 = Library.Target_Identification_Coding(Six3);
            Position = Position + 1;
            string Six4 = Convert.ToString(CAT10_Message[Position-1][2] + CAT10_Message[Position-1][3] + CAT10_Message[Position-1][4] + CAT10_Message[Position-1][5] + CAT10_Message[Position-1][6] + CAT10_Message[Position-1][7]);
            Char4 = Library.Target_Identification_Coding(Six4);
            Position = Position + 1;
            string Six5 = Convert.ToString(CAT10_Message[Position][0] + CAT10_Message[Position][1] + CAT10_Message[Position][2] + CAT10_Message[Position][3] + CAT10_Message[Position][4] + CAT10_Message[Position][5]);
            Char5 = Library.Target_Identification_Coding(Six5);
            Position = Position + 1;
            string Six6 = Convert.ToString(CAT10_Message[Position - 1][6] + CAT10_Message[Position - 1][7] + CAT10_Message[Position][0] + CAT10_Message[Position][1] + CAT10_Message[Position][2] + CAT10_Message[Position][3]);
            Char6 = Library.Target_Identification_Coding(Six6);
            Position = Position + 1;
            string Six7 = Convert.ToString(CAT10_Message[Position - 1][6] + CAT10_Message[Position - 1][7] + CAT10_Message[Position][0] + CAT10_Message[Position][1] + CAT10_Message[Position][2] + CAT10_Message[Position][3]);
            Char7 = Library.Target_Identification_Coding(Six7);
            Position = Position + 1;
            string Six8 = Convert.ToString(CAT10_Message[Position - 1][4] + CAT10_Message[Position - 1][5] + CAT10_Message[Position - 1][6] + CAT10_Message[Position - 1][7] + CAT10_Message[Position][0] + CAT10_Message[Position][1]);
        ...
        public string ST; public string Target_Identificatio;
        ...
        private int Decode_Size_Orientation(int Position, string[] CAT10_Message)
        {
            ...
            string Lenght = Convert.ToString(Convert.ToInt32(Lenght_bin.Remove(Lenght_bin.Length - 1, 1), 2)); //1m
            Position = Position + 1;
            if (Lenght_bin[7] == '1')
            {
                string Orientation_bin = Convert.ToString(CAT10_Message[Position]);
                string Orientation = Convert.ToString(Convert.ToInt32(Orientation_bin.Remove(Orientation_bin.Length - 1, 1), 2)*2.81); //º
                Position = Position + 1;

                if (Orientation_bin[7] == '1')
                {
                    string Width_bin = Convert.ToString(CAT10_Message[Position]);
                    string Width = Convert.ToString(Convert.ToInt32(Width_bin.Remove(Width_bin.Length - 1, 1), 2)); //1m
                    Position = Position + 1;
                }
            }
            return Position;
